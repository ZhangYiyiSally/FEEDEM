...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, epoch_num):
        ...
        loss_value, energy_loss, boundary_loss, driven_loss=loss.loss_function(dom, bc_Dir, bc_Pre, bc_Sym, Pre_load, loss_weight, driven_data)
        # 反向传播
        optimizer.zero_grad()
        loss_value.backward()
        # 梯度范数）
        ...
        # 更新epoch进度条
        tqdm_epoch.update()
        tqdm_epoch.set_postfix({'loss':'{:.5e}'.format(losses[-1]),'eloss':'{:.5e}'.format(energy_loss), 'bloss':'{:.5e}'.format(boundary_loss), 'dloss':'{:.5e}'.format(driven_loss),'lr':'{:.3e}'.format(lr_history[-1])})
